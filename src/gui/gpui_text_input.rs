...
pub struct TextInput {
    ...
    is_selecting: bool,
    on_change: Option<Box<dyn FnMut(String, &mut Context<Self>)>>,
}
...
impl TextInput {
    ...
    fn range_to_utf16(&self, range: &Range<usize>) -> Range<usize> {
        let start = self.content[0..range.start].chars().map(|c| c.len_utf16()).sum();
        let end = self.content[0..range.end].chars().map(|c| c.len_utf16()).sum();
        start..end
    }
...
impl EntityInputHandler for TextInput {
    ...
    fn replace_text_in_range(
        ...
        if let Some(handler) = self.on_change.as_mut() {
            handler(self.content.to_string(), cx);
        }
        cx.notify();
    }
